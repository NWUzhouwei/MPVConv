...
__global__ void grid_stats_kernel(int b, int n, int r, int r2, int r3,
  ...
  // stripe represents how many threads each batch uses for parallel computation
  int stride = blockDim.x;
  int index = threadIdx.x;
  // Coords is a pointer to the current batch
  ...
  for (int i = index; i < n; i += stride) {
    // if (ind[i] == -1)
    //   continue;
    // Calculate which voxel this point belongs to and find its subscript
    ind[i] = coords[i] * r2 + coords[i + n] * r + coords[i + n + n];
    // Add 1 to the number of interior points of the voxel
...
__global__ void avg_voxelize_kernel(int b, int c, int n, int s,
  ...
  cnt += batch_index * s;
  for (int i = index; i < n; i += stride) {
    // Find the subscript of voxel corresponding to the current point
    int pos = ind[i];
    // if (pos == -1)
    //   continue;
    // Gets the number of points in the voxel
    int cur_cnt = cnt[pos];
    if (cur_cnt > 0) {
      float div_cur_cnt = 1.0 / static_cast<float>(cur_cnt);
      for (int j = 0; j < c; j++) {
        // Update each dimension in voxel's feature
