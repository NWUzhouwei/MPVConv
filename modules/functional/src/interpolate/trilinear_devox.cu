...
__global__ void trilinear_devoxelize_kernel(int b, int c, int n, int r, int r2,
  ...
  // if not training, pointer to the wrong position, but no conduction to wgts and inds in memory
  ...
  for (int i = index; i < n; i += stride) {
    // get the coordinate of i_th points
    float x = coords[i];
    float y = coords[i + n];
    float z = coords[i + n + n];
    // compute the first point for interpolation, it is (floor(x), floor(y), floor(z))
    float x_lo_f = floorf(x);
    float y_lo_f = floorf(y);
    float z_lo_f = floorf(z);

    // since the edge of grid is 1, calculate the neigboring 8 points' distances to the i_th point
	// Attention, most points are inside the grid and are not in the edge face due to the features of point cloud
    // But voxel_coord is clamped from (0,r) to (0,r-1]
    // when x_d_1 equals to 0, it is in the R_th voxel
    ...
    // Calculate weights
	// f(x) = (ceil(x)-x) * f(floor(x)) + (x-floor(x)) * f(ceil(x))
	// wgt000 = (ceil(x)-x) * (ceil(y)-y) * (ceil(z)-z)
	// wgt000 (floor(x), floor(y), floor(z))
