...
from visualizer.visual_s3dis import s3dis_color_classes
...
def evaluate(configs=None):
    ...
    # directory of visualize
    #if configs.visual:
    visual_dir = 'visual/s3dis/'
    ...
    for scene_index, (scene, scene_files) in enumerate(tqdm(dataset.scene_list.items(), desc='eval', ncols=0)):
        ...
        # visualize
        #os.mknod(os.path.join(visual_dir, scene + '_pred.obj'))
        #os.mknod(os.path.join(visual_dir, scene + '_gt.obj'))
        #if configs.visual:
